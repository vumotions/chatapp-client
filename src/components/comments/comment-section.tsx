...
export default function CommentSection({ postId }: CommentSectionProps) {
  ...
  useEffect(() => {
    ...
    const handleNewComment = (data: { comment: Comment, isReply: boolean, creatorId: string }) => {
      console.log('Received NEW_COMMENT event:', data)
      
      // Nếu là reply thì không xử lý ở đây
      if (data.isReply) return
      
      // Nếu comment này được tạo bởi người dùng hiện tại, bỏ qua
      if (data.creatorId === session?.user?.id) {
        console.log('Comment created by current user, ignoring socket event')
        return
      }
      ...
      setLocalComments(prev => {
        // Kiểm tra trùng lặp
        const isDuplicate = prev.some(comment => 
          comment._id === data.comment._id || 
          (comment.tempId && comment.tempId === data.comment.tempId)
        )
  ...
  }, [socket, postId, session?.user?.id, disableSocket])
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-4">
        ...
        {isLoading && <p className="text-center text-muted-foreground">Đang tải bình luận...</p>}
